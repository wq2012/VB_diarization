...
def VB_diarization(X, m, iE, w, V, sp=None, q=None,
  ...
  minDur      - minimum number of frames between speaker turns imposed by linear
                chains of HMM states corresponding to each speaker. All the
                states in a chain share the same output distribution
  ...
  # for ii in range(C):
  #  VtiEF = VtiEF +
  #  V[ii*D:(ii+1)*D,:].T.dot(NN[ii,:] * np.sqrt(iE[:,[ii]]) *  (X - m[:,[ii]]))

  if downsample is not None:
    # Downsample NN, VtiEF, G and q by summing the statistic over 'downsample'
    # frames
