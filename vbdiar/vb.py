...
def VB_diarization(X, m, iE, w, V, sp=None, q=None,
    ...
    Conversations using Factor Analysis IEEE Journal of Selected Topics in
    Signal Processing, December 2010.
    ...
    1) clustering speech segments and 2) re-segmentation (i.e. as it was done
    in the paper above).

    Inputs:
    X  - T x D array, where columns are D dimensional feature vectors for T
         frames
    ...
    sparsityThr - set occupations smaller that this threshold to 0.0 (saves
                  memory as the posteriors are represented by sparse matrix)
    ...
    VtiEV       - C x (R**2+R)/2 matrix normally calculated by VB_diarization
                  when VtiEV is None. However, it can be pre-calculated using
                  function precalculate_VtiEV(V) and used across calls of
                  VB_diarization.
    minDur      - minimum number of frames between speaker turns imposed by
                  linear chains of HMM states corresponding to each speaker.
                  All the states in a chain share the same output distribution
    ref         - T dim. integer vector with reference speaker ID
                  (0:maxSpeakers) per frame
    plot        - if set to True, plot per-frame speaker posteriors.
     ...
     Li - values of auxiliary function (and DER and frame cross-entropy between
          q and reference if 'ref' is provided) over iterations.
    """

    # The references to equations corresponds to the technical report:
    # Kenny, P. Bayesian Analysis of Speaker Diarization with Eigenvoice
    # Priors, Montreal, CRIM, May 2008.
    ...
    if q is None:
        # initialize q from flat Dirichlet prior with concentrsaion parameter
        # alphaQInit
        q = np.random.gamma(alphaQInit, size=(nframes, maxSpeakers))
        q = q / q.sum(1, keepdims=True)

    # calculate UBM mixture frame posteriors
    # (i.e. per-frame zero order statistics)
    ...
    # G = np.sum((NN.multiply(ll - np.log(w))).toarray(), 1) + Kx  # eq. (15)
    # Aleready calculated above

    # Calculate per-frame first order statistics projected into the R-dim.
    # subspace V^T \Sigma^{-1} F_m
    ...
    del F_s
    # The code above is only efficient implementation of the following comented
    # code
    # VtiEF = 0;
    # for ii in range(C):
    #  VtiEF = VtiEF +
    #  V[ii*D:(ii+1)*D,:].T.dot(NN[ii,:]
    #    * np.sqrt(iE[:,[ii]]) * (X - m[:,[ii]]))

    if downsample is not None:
        # Downsample NN, VtiEF, G and q by summing the statistic over
        # 'downsample' frames
        # This speeds-up diarization for the price of lowering its frame
        # resolution
        downsampler = coo_matrix((np.ones(nframes), (np.ceil(
            np.arange(nframes)/downsample).astype(int), np.arange(nframes))))
    ...
    for ii in range(maxIters):
        ...
        Ns = NN.T.dot(q).T
        VtNsiEV_flat = Ns.astype(VtiEV.dtype).dot(
            VtiEV)  # eq. (34) except for 'I' for all 's'
        # eq. (35) except for \Lambda_s^{-1} for all 's'
        VtiEFs = q.T.dot(VtiEF)
        for sid in range(maxSpeakers):
            invL = np.linalg.inv(
                np.eye(R) + tril_to_sym(VtNsiEV_flat[sid]))  # eq. (34) inverse
            # eq. (35)
            a = invL.dot(VtiEFs[sid])
            # eq. (29) except for the prior term \ln \pi_s. Our prior is given
            # by HMM trasition probability matrix. Instead of eq. (30), we need
            # to use forward-backwar algorithm to calculate per-frame speaker
            # posteriors, where 'lls' plays role of HMM output
            # log-probabilities
        ...
        # states for each of 'maxSpeaker' speaker. The last state in each chain
        # has self-loop probability 'loopProb' and the transition probabilities
        # to the initial chain states given by vector '(1-loopProb) * sp'.
        # From all other, states, one must move to the next state in the chain
        # with probability one.
        tr[minDur-1::minDur, 0::minDur] = (1-loopProb)*sp
        tr[(np.arange(1, maxSpeakers+1)*minDur-1,)*2] += loopProb
        ip[::minDur] = sp
        # per-frame HMM state posteriors. Note that we can have linear chain of
        # minDur states for each speaker.
        # , np.arange(1,maxSpeakers+1)*minDur-1)
        q, tll, lf, lb = forward_backward(lls.repeat(minDur, axis=1), tr, ip)

        # Right after updating q(Z), tll is E{log p(X|,Y,Z)} - KL{q(Z)||p(Z)}.
        # L now contains -KL{q(Y)||p(Y)}. Therefore, L+ttl is correct value for
        # ELBO.
        L += tll
        Li.append([L])
        ...
        # per-frame speaker posteriors (analogue to eq. (30)), obtained by
        # summing HMM state posteriors corresponding to each speaker
        q = q.reshape(len(q), maxSpeakers, minDur).sum(axis=2)

        # if reference is provided, report DER, cross-entropy and plot the
        # figures
        if ref is not None:
            Li[-1] += [DER(downsampler.T.dot(q), ref),
                       DER(downsampler.T.dot(q), ref, xentropy=True)]
            ...
            if plot:
                ...
                matplotlib.pyplot.imshow(
                    np.atleast_2d(ref), interpolation='none', aspect='auto',
...
# reference (vector of per-frame zero based integer speaker IDs) and q
# (per-frame speaker posteriors). If expected=False, q is converted into hard
# labels before calculating DER. If expected=TRUE, posteriors in q are used to
# calculated "expected" DER.
...
def forward_backward(lls, tr, ip):
    ...
    Inputs:
        ...
        ip  - vector of initial state probabilities (i.e. statrting in the
              state)
    Outputs:
